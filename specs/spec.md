...
- Persisted invalid global shortcut strings **MUST NOT** be registered as OS shortcuts on startup; malformed legacy values **MUST** be reset to their default keybinds before registration proceeds.
...
- Shortcut capture UI keyboard activation **MUST** treat both `' '` and `U+00A0` key events as Space so `Opt+Space` can be captured reliably.
