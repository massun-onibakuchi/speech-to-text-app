...
description: Route local whisper.cpp Core ML streaming from the selected STT provider and model instead of introducing a separate processing mode.
date: 2026-03-18
status: accepted
tags:
  - architecture
...
The local whisper.cpp Core ML feature adds a new local provider with different runtime behavior:
...
- model installation and Core ML preparation become part of session startup
...
- selecting `transcription.provider=local_whispercpp_coreml` routes recording commands to the local streaming lane
