...
describe('createCaptureProcessor', () => {
  ...
  it('falls back to transcript when transformation returns whitespace-only text', async () => {
    const applyOutputWithDetail = vi.fn(async () => ({ status: 'succeeded' as TerminalJobStatus, message: null }))
    const deps = makeDeps({
      transformationService: {
        transform: vi.fn(async () => ({
  ...
  it('preserves selected transformed destinations when falling back to transcript text', async () => {
    const applyOutputWithDetail = vi.fn(async () => ({ status: 'succeeded' as TerminalJobStatus, message: null }))
    const deps = makeDeps({
      transformationService: {
        transform: vi.fn(async () => ({
