...
expected_mount='source=${localEnv:HOME}/.config/git/.gitconfig,target=/home/node/.gitconfig,type=bind,consistency=cached,readonly'
...
' "${config_file}" >/dev/null; then
  echo "ERROR: expected .config/git/.gitconfig mount missing from ${config_file}" >&2
  exit 1
fi

echo "OK: ${config_file} uses the expected host .config/git/.gitconfig mount"
